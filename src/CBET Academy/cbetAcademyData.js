...
export const missionTenLessons = [
  {
    ...
    check: { question: "What is the most reliable way to identify a medical gas source?", options: ["Cylinder color only", "Room location", "Verify the product label and indexed connection", "The hose length"], answer: 2, explanation: "Labels and indexed connections are the primary identification safeguards; color alone is not enough." },
  },
  {
...
export const missionTenScenarios = [
  {
    title: "Transport Cylinder: Not Enough Reserve",
    patient: "A transport team is preparing to move a patient with an E-cylinder. The pressure gauge reads approximately 250 PSI. The trip includes imaging, waiting time, and return transport.",
    question: "What is the strongest Clinical Engineering recommendation?",
    options: ["Proceed because the gauge still shows positive pressure", "Replace the cylinder with one that meets the facility's transport-reserve requirement", "Increase the regulator flow so pressure remains stable", "Remove the regulator and estimate the remaining contents manually"],
    answer: 1,
    explanation: "The issue is not whether gas remains; it is whether adequate reserve exists for the expected clinical use. Follow the facility's transport policy and verify sufficient supply before departure.",
  },
  {
    title: "Vacuum Regulator: No Suction at One Bed",
    patient: "A nurse reports no suction at one bed. The regulator is connected, but the gauge does not respond. A second room on the same unit has normal wall suction.",
    question: "Which next step gives the most useful evidence?",
    options: ["Escalate immediately as a central vacuum-pump failure", "Verify outlet engagement, regulator mode, tubing, canister seal, and compare with known-good point-of-use equipment", "Increase oxygen flow to the patient's room", "Replace the wall outlet before testing the regulator"],
    answer: 1,
    explanation: "Normal suction elsewhere makes a central failure less likely. Isolate the local outlet, regulator, tubing, canister, and setup before escalating infrastructure.",
  },
  {
    title: "Oxygen-Air Blender: Alarm on Setup",
    patient: "An oxygen-air blender alarms immediately after setup. Oxygen is connected. The air hose appears inserted, but the alarm persists and delivered concentration is unstable.",
    question: "What should be established before replacing the blender?",
    options: ["Whether both source connections are fully engaged and delivering the expected supply", "Whether the blender knob can be turned beyond its normal stop", "Whether the oxygen hose can temporarily feed both inlets", "Whether the alarm can be disabled during the procedure"],
    answer: 0,
    explanation: "A blender depends on both source gases. Verify source availability, proper connection, and the point-of-use supply before condemning the blender.",
  },
  {
    title: "Multiple Rooms: Low Oxygen Pressure",
    patient: "Several rooms in the same clinical zone report low oxygen pressure at nearly the same time. Different flowmeters and hoses are in use, and the problem does not follow one accessory.",
    question: "What does the failure pattern suggest?",
    options: ["A single defective bedside flowmeter", "A broader pipeline or zone-level issue that should be escalated through the facility response process", "Every oxygen hose failed simultaneously", "The cylinder pin-index system is misconfigured"],
    answer: 1,
    explanation: "A multi-room pattern shifts suspicion from a point-of-use accessory toward shared infrastructure. CE should document the evidence and escalate according to facility procedure rather than opening central pipeline components outside its scope.",
  },
  {
    title: "Cylinder Yoke: Leak After Connection",
    patient: "A portable oxygen cylinder begins leaking audibly at the yoke immediately after a cylinder change. The valve is closed again and the area is safe.",
    question: "What is the best next action?",
    options: ["Apply oil to the yoke seal so it seats more easily", "Inspect the correct seal/washer, yoke alignment, indexed connection, and mating surfaces before reconnecting", "Tighten the yoke with a tool until the leak stops", "Use thread tape around the cylinder valve outlet"],
    answer: 1,
    explanation: "Do not introduce oil, grease, improvised sealants, or excessive force into oxygen service. Inspect the approved connection components and correct the cause before reuse.",
  },
  {
    title: "Regulator: Pressure Creep",
    patient: "A cylinder regulator initially stabilizes at the expected outlet pressure. With downstream flow stopped, the outlet pressure slowly continues to rise.",
    question: "Which finding is most important?",
    options: ["The regulator may not be controlling downstream pressure correctly and should be removed from service for evaluation", "The cylinder must be nearly empty", "The flowmeter should be laid horizontally to reduce pressure", "The gauge can be ignored because no gas is flowing"],
    answer: 0,
    explanation: "A regulator that does not hold controlled outlet pressure can represent an internal regulator fault. Remove it from clinical use and evaluate it using the approved procedure.",
  },
  {
    title: "Flowmeter: Reading Changes With Position",
    patient: "A Thorpe-tube flowmeter appears normal while upright, but the indicated flow changes when the assembly is tilted during equipment movement.",
    question: "What is the best interpretation?",
    options: ["Orientation can affect the float reading, so verify the device in its intended operating position", "The oxygen source has changed gas composition", "The wall outlet pressure must be zero", "The indexed fitting has converted to another gas service"],
    answer: 0,
    explanation: "Variable-area flowmeters depend on proper orientation for meaningful float position. Verify in the manufacturer's intended operating position.",
  },
  {
    title: "Wrong Connector: It Almost Fits",
    patient: "A replacement hose will not fully engage a medical-gas outlet. The fitting looks similar to the original, and a user asks whether it can be modified slightly to fit.",
    question: "What is the correct response?",
    options: ["Modify the fitting only enough to make the connection", "Do not alter the indexed connector; verify the correct gas-specific hose and connection type", "Use an adapter made from general-purpose plumbing parts", "Remove the indexing feature if the hose is clearly labeled"],
    answer: 1,
    explanation: "Gas-specific connector systems are designed to reduce hazardous misconnections. Do not defeat that safeguard by filing, adapting, or altering the connection.",
  },
];
...
export const missionTenQuestions = [
  ...
  ["What should you do if a quick-connect fitting will not engage?", ["Force it", "Verify the fitting type and inspect both mating surfaces", "Apply oil", "File the connector"], 1, "Never force or alter an indexed medical gas connection."],
  ...
  ["What information should a Clinical Engineer document after troubleshooting?", ["Only the room number", "Complaint, findings, actions, verification results, and disposition", "Only the part cost", "Nothing if no part was replaced"], 1, "Complete documentation supports safety, continuity, and future troubleshooting."],
